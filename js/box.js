  ...
  function box(g) {
    g.each(function(d, i) {
      ...
      whisker.enter().insert("line", "circle, text")
          ...
          .attr("x2", .75*width)
          .attr("y2", x0)
          .attr('stroke',function(i){return holc_colors[i]})
          .style("opacity", 1e-6)
        .transition()
          .duration(duration)
          .attr("y1", x1)
      ...
      /// Skip the 25% and 75% quantiles
      // var quartileDataNew = quartileData;
      // $.each(quartileData,function(i,d){
      //    if (i==1){quartileDataNew[0]=d} else {quartileDataNew[i]=''}
      // });
      var quartileDataNew=[quartileData[1]];
      // Update box ticks.
      var boxTick = g.selectAll("text.box")
          .data( quartileDataNew);
      // console.log(quartileDataNew);

      boxTick.enter().append("text")
          .attr("class", "box")
          .attr("dy", ".3em")
          .attr("dx",  function(d, i) { return i & 0 ? 6 : -6 })
          .attr("x", function(d, i) { return i & 0 ? width :0 })
          .attr("y", x0)
