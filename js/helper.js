...
function addSlider(s){
  var col = s.column
  var title = s.title
  $('.weights').after((`<div><h5><b >${title}</b></h5><div class= 'nstSlider' id =${col} data-range_min="0" data-range_max="100" data-cur_min="50">
...
function addCheckbox(s){
  console.log('SETTING UP CHECKBOX ', s)
  var col = s.column
  var title = s.title
  var t = `<input type="checkbox" id="${col}"> ${title}<br><br>`
...
function calcButton(s){
  $('.calculate').after(`<p></p><button class="ui button" id='calc'><span>Calculate</span></button>`);
...
function addExtraLayer(s){
  ...
  var layerColor = s.colorCSS;
  var condition = s.condition;
  var layerSource = new carto.source.Dataset(s.name);
  ...
  var layer = new carto.layer.Layer(layerSQLSource, layerStyle);
  var client = new carto.Client({
  ...
  // Add layer
  $(`<div class="ui checkbox ${s.name}">
    <input type="checkbox" name=${s.name}>
  <label>
    <svg width="20" height="15">
    <rect x="5"y = '2' width="13" height="13" style="fill:${legendColor}" />
    </svg>
    ${s.title}
  </label>
  ...
  $checked = $('.layerSelector').find('.ui.checkbox#'+s.name+' label');


  $('.ui.checkbox.'+s.name).checkbox({
      // name: s.name,
      // label: s.title,
      fireOnInit: true,
      onChecked: function () {
  ...
  layer.on('featureOver', featureEvent => {
    ...
    popup_cust.setContent(`Category: ${String(featureEvent.data.facdomain)}`);
